...
- [x] Direct-call учителю: timeout.
- [x] Закрытое приложение: входящий звонок до истечения 90 секунд.
- [x] Закрытое приложение: заявка удаляется после 90 секунд без heartbeat.
- [x] Таймаут входа в комнату 60 секунд.
- [x] Завершение звонка и очистка очереди.
...
- [x] Запустить релевантные Flutter-тесты.
- [x] Запустить релевантные backend-тесты.
- [x] Проверить, что в студенческом интерфейсе нет `availabilityToday`.
- [x] Проверить, что учительская доступность не сломана.
- [x] Проверить, что старые звонки и история звонков открываются корректно.
- [x] Проверить, что ошибки не оставляют пользователя в активном поиске.
- [x] Проверить логи backend на сценариях accept, decline, timeout и cleanup.
