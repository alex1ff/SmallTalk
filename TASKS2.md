...
- [ ] Убрать из студенческого дашборда переключатель "Доступен сегодня".
- [ ] Убрать из студенческого интерфейса расписание доступности.
- [ ] Убрать возможность менять `availabilityToday` из студентского интерфейса.
- [ ] Оставить `availabilityToday` только для учителей.
- [ ] Обновить проверки и компоненты, которые ожидают наличие student availability UI.
- [ ] Проверить, что учительский интерфейс доступности не изменился.
...
- [ ] Добавить кнопку "Начать поиск" в студенческий интерфейс.
- [ ] Добавить состояние "Остановить поиск" после запуска поиска.
- [ ] Добавить экран или блок "Ищем собеседника".
- [ ] Добавить состояние "Соединяем".
- [ ] Добавить состояние "Пока никого не нашли" после 10 минут без пары.
- [ ] Добавить обработку ошибок поиска.
- [ ] Добавить проверку доступа перед стартом поиска: авторизация, подписка, лимиты, активный звонок, камера и микрофон.
- [ ] При ручной остановке поиска вызывать backend-остановку активной заявки.
...
- [ ] Создать структуру активной заявки поиска.
- [ ] Реализовать backend-функцию `startSearch`.
- [ ] Реализовать backend-функцию `stopSearch`.
- [ ] Реализовать backend-функцию `heartbeatSearch`.
- [ ] Сохранять в заявке пользователя, язык, фильтры, статус, `createdAt`, `heartbeatAt`, `expiresAt`, состояние приложения.
- [ ] Обновлять heartbeat каждые 30 секунд с клиента.
- [ ] Удалять или помечать неактивной заявку при отсутствии heartbeat 90 секунд.
- [ ] Останавливать поиск через 10 минут без найденной пары.
- [ ] Останавливать поиск в фоне через 10 минут.
- [ ] Гарантировать, что у одного студента может быть только одна активная заявка.
...
- [ ] Собрать единый пул кандидатов: активные студенты из очереди и доступные учителя.
- [ ] Для студентов проверять активную заявку поиска.
- [ ] Для учителей проверять расписание доступности.
- [ ] Исключать учителей без активного VoIP/push токена.
- [ ] Проверять язык звонка.
- [ ] Проверять уровень языка: сначала точное совпадение, затем ближайший уровень на один шаг выше или ниже.
- [ ] Проверять страну или город, если фильтр выбран.
- [ ] Проверять блокировки в обе стороны.
- [ ] Проверять `isInCall`.
- [ ] Проверять право пользователя участвовать в звонке.
- [ ] Проверять защиту от повторного подбора.
- [ ] Отсортировать кандидатов по качеству совпадения фильтров и времени ожидания.
- [ ] Исключить приоритет роли кандидата из алгоритма.
...
- [ ] Реализовать атомарную блокировку двух участников при создании пары.
...
- [ ] Возвращать второго участника в поиск, если пара сорвалась до начала звонка.
- [ ] Защитить сценарии от гонок при одновременном поиске нескольких пользователей.
...
- [ ] При найденной паре student-student создать общую сессию.
- [ ] Если оба студента в открытом приложении, автоматически открыть страницу звонка у обоих.
- [ ] Если один студент в фоне, отправить ему системный входящий звонок.
- [ ] Обработать accept входящего звонка.
- [ ] Обработать decline входящего звонка.
- [ ] Обработать timeout 45 секунд без ответа.
- [ ] При отказе или timeout удалить неответившего студента из очереди.
- [ ] Вернуть второго студента в активный поиск.
...
- [ ] При выборе учителя отправлять ему системный входящий звонок.
- [ ] Оставлять студента на экране ожидания соединения.
- [ ] При accept учителя переводить обоих участников на страницу звонка.
- [ ] При decline учителя исключать его из текущей попытки.
- [ ] При timeout 45 секунд исключать учителя из текущей попытки.
- [ ] После отказа или timeout продолжать подбор по общему пулу.
- [ ] Не давать роли следующего кандидата отдельный приоритет.
...
- [ ] Оставить прямой звонок из профиля учителя отдельным сценарием.
- [ ] Не добавлять direct-call в общую очередь поиска.
- [ ] Проверять доступность учителя по расписанию.
- [ ] Проверять активный VoIP/push токен учителя.
- [ ] Проверять блокировки, `isInCall` и базовые права доступа.
- [ ] При accept открывать страницу звонка у студента и учителя.
- [ ] При decline или timeout завершать direct-call без автоперехода в общий поиск.
...
- [ ] Фиксировать факт входа каждого участника в комнату звонка.
- [ ] Запускать таймаут 60 секунд после подтверждения пары.
- [ ] Если оба вошли в комнату за 60 секунд, переводить сессию в `active`.
- [ ] Если один участник не вошел, отменять пару.
- [ ] Возвращать ожидавшего участника в активный поиск.
- [ ] Снимать `isInCall` с участников при отмене до старта звонка.
...
- [ ] При старте приложения проверять активный поиск пользователя.
...
- [ ] При завершении звонка удалять обеих участников из активного поиска.
- [ ] Снимать `isInCall` с обоих участников.
- [ ] Переводить сессию в `ended`.
- [ ] Открывать стандартный экран завершения или оценки звонка.
- [ ] Не запускать повторный поиск автоматически.
- [ ] Проверить cleanup для отмененных и истекших сессий.
...
- [ ] Тест `startSearch`: создается одна активная заявка.
- [ ] Тест `stopSearch`: заявка останавливается.
- [ ] Тест `heartbeatSearch`: heartbeat продлевает активность.
- [ ] Тест 90 секунд без heartbeat: заявка истекает.
- [ ] Тест 10 минут без пары: поиск останавливается.
- [ ] Тест единого пула: студент и учитель сравниваются без приоритета роли.
- [ ] Тест уровня: точный уровень выбирается раньше ближайшего.
- [ ] Тест уровня: ближайший уровень допускается только на один шаг.
- [ ] Тест блоклистов в обе стороны.
- [ ] Тест исключения учителя без VoIP/push токена.
- [ ] Тест student-student пары.
- [ ] Тест student-teacher пары.
- [ ] Тест decline и timeout с возвратом второго участника в поиск.
- [ ] Тест защиты от двойной сессии для одного пользователя.
...
- [ ] Тест отсутствия student availability UI.
- [ ] Тест отображения кнопки "Начать поиск".
- [ ] Тест перехода кнопки в "Остановить поиск".
- [ ] Тест состояния "Ищем собеседника".
- [ ] Тест состояния "Соединяем".
- [ ] Тест состояния "Пока никого не нашли".
- [ ] Тест ошибки доступа: нет подписки или лимита.
- [ ] Тест ошибки доступа: нет камеры или микрофона.
- [ ] Тест восстановления активного поиска после перезапуска.
- [ ] Тест восстановления активного звонка после перезапуска.
...
- [ ] Student-student: оба приложения открыты.
- [ ] Student-student: один студент в фоне.
- [ ] Student-student: один студент отклоняет звонок.
