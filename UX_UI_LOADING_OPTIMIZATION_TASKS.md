...
- [x] Не пересобирать весь список сообщений после отправки одного сообщения.
- [x] Зафиксировать высоту input-bar и send button.
- [x] Убрать скачок нижней панели при открытии клавиатуры и safe area.
- [x] Убрать лишние проверки доступа из UI-цепочки отправки, если пользователь уже находится в доступном чате.

## UX Phase 4: Events List

- [x] Сохранить последний список событий по ключу `city + dateFilter + levelFilters`.
- [x] При смене фильтра оставлять старый список до прихода нового результата.
- [x] Показывать компактный refresh indicator вместо очистки списка.
- [x] Empty state показывать только после завершения загрузки нового фильтра.
- [x] Зафиксировать высоту и структуру карточки события.
- [x] Не менять размеры карточки при догрузке участников и аватаров.
...
- [x] Не показывать blank-заглушку участника: если профиль уже был известен, сохранять его; если известен только ref, показывать стабильный fallback avatar/initial slot.
- [x] Зафиксировать размеры блоков организатора, даты/времени/места и участников.
- [x] Sticky bottom action bar не должен прыгать при обновлении данных.
- [x] Ошибка обновления detail не должна стирать уже показанное событие.

## UX Phase 6: Dictionary

- [x] Убрать отдельные bordered cards у слов, заменить на строки списка с divider.
- [x] Зафиксировать высоту строки слова.
- [x] Разнести оригинал и перевод так, чтобы длинный текст не слипался.
- [x] Нижнюю панель повторения сделать стабильной по высоте.
- [x] Сократить текст счетчика повторения, чтобы он не обрезался на больших числах.
- [x] Проверить состояния пусто/загрузка/ошибка без скачков.

## UX Phase 7: Profile And Secondary Lists

- [x] Профиль сначала показывает данные из `currentUserDocument`, вторичные данные догружаются отдельно.
- [x] Блоки профиля не должны менять размер при догрузке статистики, подписки и почты.
- [x] Аватар профиля не должен прыгать между placeholder и реальным состоянием.
- [x] Проверить списки истории, моих событий, черного списка и друзей на мигание empty/error.
- [x] Для каждого вторичного списка применить правило `previousData + refreshing indicator`.
...
- [x] Запустить релевантные `flutter test`.
