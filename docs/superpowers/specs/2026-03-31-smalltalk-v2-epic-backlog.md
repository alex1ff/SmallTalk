...
- This run: an additive `participantIds` migration slice now lands ahead of the broader all-to-all rollout. New and accepted sessions write `participantIds`; Firestore rules still preserve legacy `studentId`/`tutorId`/`currentTutorId` access during migration, while backend session callables fall back only to legacy `studentId`/`tutorId`.
- This run: participant-scoped access is hardened without widening pending-tutor token access and without pretending that the broader `Issue 5.1`/`Issue 5.2` session normalization is finished.
- Prior 2026-04-14 hardening: scheduled cleanup now writes the same repeat-history metadata as manual `endSession`, using the session `expiresAt` value for day-key bucketing and a fresh transaction re-read before commit.
- Remaining Epic 5 scope is the broader all-to-all ranking/profile/session normalization work, plus higher-fidelity backend verification before `Issue 5.4` should be treated as fully closed.
