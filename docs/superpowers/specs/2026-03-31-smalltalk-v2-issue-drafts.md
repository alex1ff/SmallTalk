...
- New searching sessions now write additive `participantIds` for the student, and accepted sessions expand that field to the confirmed student/tutor pair.
- Firestore rules plus `getSessionTokens`, `getDeepgramToken`, `endSession`, and `submitReview` now honor `participantIds` with legacy fallback, so migration can proceed without breaking existing session docs.
- Client-side session updates are not allowed to rewrite `studentId`, `tutorId`, `currentTutorId`, or `participantIds`; those access fields are now treated as server-managed authorization data.
- Pending tutors still rely on `currentTutorId`; this slice does not treat a notified-but-unaccepted tutor as an active session participant.
- `Issue 5.3` remains open until the broader `Issue 5.1` / `Issue 5.2` session generalization is complete and post-change review/session parity is re-verified.
...
Prevent matching the same pair twice in one calendar day after a completed connected conversation, while allowing configured tester users to bypass this for QA with Alexander/client testing.
...
- Tester allow-list bypasses the exclusion
...
- This run hardens scheduled cleanup parity but does not yet replace emulator-backed/backend end-to-end proof for closing `Issue 5.4`.
