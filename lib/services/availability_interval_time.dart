...
class AvailabilityIntervalRange {
  ...
  bool operator ==(Object other) =>
      other is AvailabilityIntervalRange &&
...
int? parseAvailabilityIntervalMinutes(String? rawValue) {
  final match = RegExp(r'^(\d{1,2}):(\d{2})$').firstMatch(rawValue ?? '');
  if (match == null) {
    return null;
  }
...
String formatAvailabilityIntervalMinutes(int minutes) {
  final validMinutes = clampAvailabilityIntervalMinutes(
    minutes,
    min: 0,
    max: kAvailabilityIntervalLastMinuteOfDay,
  );
  final hours = (validMinutes ~/ 60).toString().padLeft(2, '0');
  final minute = (validMinutes % 60).toString().padLeft(2, '0');
  return '$hours:$minute';
}

List<AvailabilityIntervalRange> mergeAvailabilityIntervalRanges(
  Iterable<AvailabilityIntervalRange> intervals,
) {
  final sortedIntervals = intervals
      .map(
        (interval) => AvailabilityIntervalRange(
          startMinutes: clampAvailabilityIntervalMinutes(
            interval.startMinutes,
            min: 0,
            max: kAvailabilityIntervalLastMinuteOfDay,
          ),
          endMinutes: clampAvailabilityIntervalMinutes(
            interval.endMinutes,
            min: 0,
            max: kAvailabilityIntervalLastMinuteOfDay,
          ),
  ...
  final mergedIntervals = <AvailabilityIntervalRange>[];
  for (final interval in sortedIntervals) {
    ...
    final previous = mergedIntervals.last;
    if (interval.startMinutes <= previous.endMinutes) {
      mergedIntervals[mergedIntervals.length - 1] = AvailabilityIntervalRange(
        startMinutes: previous.startMinutes,
        endMinutes: interval.endMinutes > previous.endMinutes
            ? interval.endMinutes
            : previous.endMinutes,
      );
      continue;
